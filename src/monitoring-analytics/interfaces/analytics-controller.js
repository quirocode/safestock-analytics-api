class AnalyticsController {
  constructor(service, subscriptionService) {
    ...
    for(const method of ['summary','lowStock','suspicious','advanced','consolidated','stockZeroAlerts','exportSales']) this[method]=this[method].bind(this);
  }
  ...
  async suspicious(req,res,next){try{const plan=await this.subscriptionService.assertFeature(req.user.organizationId,'antifraud');res.json(await this.service.suspicious(plan.fraudThreshold||0));}catch(error){next(error);}}
