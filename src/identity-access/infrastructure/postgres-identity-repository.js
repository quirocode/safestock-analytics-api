...
class PostgresIdentityRepository extends IdentityRepositoryPort {
  ...
  async createUser({ correo, passwordHash, nombres, apellidos, rolNombre, estado }) {
    const result = await this.database.query(
      `INSERT INTO usuarios (rol_id, correo, password_hash, nombres, apellidos, estado)
       SELECT id, $1, $2, $3, $4, $5 FROM roles WHERE nombre = $6
       RETURNING id, correo, nombres, apellidos, estado, rol_id, creado_en`,
      [correo, passwordHash, nombres, apellidos, estado, rolNombre]
    );
    return result.rows[0] || null;
  }
